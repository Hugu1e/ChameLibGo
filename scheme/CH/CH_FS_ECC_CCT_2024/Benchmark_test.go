...
func run_scheme_benchmark(t *testing.T, cur curve.Curve, group pbc.Field) {
	...
	for i := 0; i < repeat; i++ {
		m1[i] = pp[i].GetGroupElement()
		m2[i] = pp[i].GetGroupElement()
	}
	...
	timer.Start("Adapt")
	for i := 0; i < repeat; i++ {
		r1_p[i] = Adapt(h1[i], r1[i], pp[i], pk[i], sk[i], m1[i], m2[i])
	}
	timer.End("Adapt")
...
func TestCH_FS_ECC_CCT_2024(t *testing.T) {
	curs := []curve.Curve{
		curve.A,
		curve.A1,
		curve.D_159,
