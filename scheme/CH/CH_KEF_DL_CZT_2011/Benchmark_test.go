...
func run_scheme_benchmark(t *testing.T, cur curve.Curve, group pbc.Field) {
	...
	timer.Start("Hash")
	for i := 0; i < repeat; i++ {
		h1[i], r1[i] = Hash(pp[i], pk[i], L1[i], m1[i])
	}
	timer.End("Hash")
	...
	timer.Start("Check")
	for i := 0; i < repeat; i++ {
		checkRes[i] = Check(h1[i], r1[i], pp[i], pk[i], L1[i], m1[i])
	}
	timer.End("Check")
	for i := 0; i < repeat; i++ {
		if !checkRes[i] {
	...
	timer.Start("Adapt")
	for i := 0; i < repeat; i++ {
		r1_p[i] = Adapt(r1[i], pp[i], pk[i], sk[i], L1[i], m1[i], m2[i])
	}
	timer.End("Adapt")

	for i := 0; i < repeat; i++ {
		if !Check(h1[i], r1_p[i], pp[i], pk[i], L1[i], m2[i]) {
...
func TestCH_KEF_DL_CZT_2011(t *testing.T) {
	curs := []curve.Curve{
		curve.A,
		curve.A1,
		curve.D_159,
