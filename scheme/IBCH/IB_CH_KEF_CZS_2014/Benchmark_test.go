...
func run_scheme_benchmark(t *testing.T, curve curve.Curve, swap bool) {
	...
	ID2 := make([]*pbc.Element, repeat)
	for i := 0; i < repeat; i++ {
		ID1[i] = pp[i].GetZrElement()
		ID2[i] = pp[i].GetZrElement()
	}
	...
	timer.Start("Hash")
	for i := 0; i < repeat; i++ {
		h1[i], r1[i] = Hash(pp[i], ID1[i], L1[i], m1[i])
	}
	timer.End("Hash")
	...
	timer.Start("Check")
	for i := 0; i < repeat; i++ {
		checkRes[i] = Check(h1[i], r1[i], pp[i], L1[i], m1[i])
	}
	timer.End("Check")
	for i := 0; i < repeat; i++ {
		if !checkRes[i] {
	...
	timer.Start("Adapt")
	for i := 0; i < repeat; i++ {
		r1_p[i] = Adapt(r1[i], pp[i], sk1[i], L1[i], m1[i], m2[i])
	}
	timer.End("Adapt")

	for i := 0; i < repeat; i++ {
		if !Check(h1[i], r1_p[i], pp[i], L1[i], m2[i]) {
...
func TestIB_CH_KEF_CZS_2014(t *testing.T) {
	cases := []struct {
		cur curve.Curve
		swap bool
	}{
